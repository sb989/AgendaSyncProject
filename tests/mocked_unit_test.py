...
class MockedUnitTestCase(unittest.TestCase):
    ...
    def test_create_update_all_messages(self):
        with patch("googleapiclient.discovery.Resource") as build, \
        patch("calendar_helper_functions.seperate_events_by_month") as seperate:
            calendar_helper_functions.create_update_all_message('cred', DATE, DATE)
            seperate.assert_called_once()
